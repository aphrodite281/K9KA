...
let rawbodyModels = ModelManager.loadPartedRawModel(
	Resources.manager(),
	Resources.id("mtr:k9ka/model/body.obj"),
	null
);
let slogan = ModelManager.loadRawModel(
	Resources.manager(),
	Resources.id("mtr:k9ka/model/slogan.obj"),
	null
);
...
const cpd = Resources.readBufferedImage(
...
function create(ctx, state, train) {
	...
	for (let i = 0; i < train.trainCars(); i++) {
		...
		for (let j = 0; j < indoorLights.length; j++) {
			state.InternalLampStatus[i][j] =
				Math.random() < lightingRatio ? "On" : "Off";
		}
	}
...
function render(ctx, state, train) {
	...
	for (let i = 0; i < train.trainCars(); i++) {
		...
		ctx.drawCarModel(
			bodyModels.get(train.isOnRoute() ? "indoorOn" : "indoorOff"),
			i,
			matrices
		);
		ctx.drawCarModel(
			bodyModels.get(train.isOnRoute() ? "headLampOn" : "headLampOff"),
			i,
			matrices
		);
		...
		ctx.drawCarModel(
			bodyModels.get("brakeLamp" + (inBrake(state, train) ? "On" : "Off")),
			i,
			matrices
		);
		ctx.drawCarModel(
			bodyModels.get(
				"fogLamp" +
					(train.isOnRoute() && MinecraftClient.worldIsRaining() ? "On" : "Off")
			),
			i,
			matrices
		);
		if (getModelKey(train, i).includes(keywords[3])) {
			...
			(state.turning[i] == 0 && !train.isReversed()) ||
			(state.turning[i] == 2 && train.isReversed())
		) {
			state.swerveing[0] = true;
		...
		ctx.drawCarModel(
			bodyModels.get(
				"swerveLeft" +
					(state.swerveing[0] ? state.swerves[0].stateNow() : "Off")
			),
			i,
			matrices
		);
		ctx.drawCarModel(
			bodyModels.get(
				"swerveRight" +
					(state.swerveing[1] ? state.swerves[1].stateNow() : "Off")
			),
			i,
			matrices
		);
		for (let j = 0; j < indoorLights.length; j++) {
			let model = bodyModels.get(
				indoorLights[j] +
					(train.isOnRoute() ? state.InternalLampStatus[i][j] : "Off") +
					""
			);
		...
		let rot = Math.min(
			((train.speed() * 20 * 3.6) / dMaxSpeed) * (dMaxRot - dMinRot) + dMinRot,
			dMaxRot
		);
		matrices.pushPose();
		...
		ctx.drawCarModel(
			bodyModels.get("dromometer" + (train.isOnRoute() ? "On" : "Off")),
			i,
			matrices
		);
		matrices.popPose();
